...
> [!NOTE]
> **開発プロセスに関する注釈**:
> 私は日本人であるため、まず日本語で詳細な仕様書を作成しました（この `original.md` がその原本です）。
> トークン圧縮のプロセスは、英語を介さず、この**日本語の仕様書から直接、アセンブラ的なシンボリック・トークン（`compress.md`）へ一気に変換**しています。
> 日本語の技術的概念を直接シンボリック・コードへ「発色（Synthesis）」させることで、情報の欠落を抑え、極限の開発速度を実現しています。
